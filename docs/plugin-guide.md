...
中文文档站（本地）：参见 [启动与维护说明](site/README.md)。
...
/tmp/pjrt-sim-venv/bin/python tests/python/smoke_test.py -v

# Start a new process with four simulated devices and real overlap scheduling.
export PJRT_SIM_DEVICE_COUNT=4
/tmp/pjrt-sim-venv/bin/python tests/python/sglang_smoke_test.py --tp-size 4 --overlap
```
...
See [the iteration plan](performance-plan.md) for the remaining scheduling,
...
```sh
bazel build -c opt //:pjrt_sim_plugin.so
export PJRT_SIM_DEVICE_COUNT=4
/tmp/pjrt-sim-venv/bin/python tests/python/sglang_smoke_test.py --tp-size 4 --overlap \
...
```sh
python3 python/report.py /tmp/my-sim-run.123.jsonl \
  --hardware configs/v7x.json --output /tmp/report.json
```
...
```sh
bazel-bin/plan_export 4 < original.program.json > replanned.program.json
```
...
```sh
python python/replay.py /tmp/sim-profile /tmp/my-sim-run.123.jsonl \
  --scenario configs/replay_scenario.json --output /tmp/replay
python python/replay.py /tmp/sim-profile /tmp/my-sim-run.123.jsonl \
...
```sh
bazel build -c opt //:xplane_descriptor
```
...
```sh
python python/device_load.py /tmp/my-sim-run.123.program10.json \
