...
```sh
python python/replay.py /tmp/my-capture/profile \
  /tmp/my-capture/execution.123.jsonl \
  --scenario configs/replay_scenario.json \
  --output /tmp/replay
```
...
```sh
python python/device_load.py /tmp/my-capture/execution.123.program10.json \
