...
```sh
# 单设备虚拟存储。
PJRT_SIM_DEVICE_COUNT=1 python tests/python/virtual_storage_test.py -v

# 虚拟张量并行；可换成 4 或 8。
PJRT_SIM_DEVICE_COUNT=2 python tests/python/virtual_multidevice_test.py -v

# 独立 Python 成本与重放测试。
python tests/python/replay_test.py -v
python tests/python/virtual_clock_test.py -v
python tests/python/device_load_test.py -v
```
...
测试配置、主机内存观察和完整验证记录保留在 `docs/iterations.md`。主机内存与耗时观察不能作为 TPU 性能数据。
